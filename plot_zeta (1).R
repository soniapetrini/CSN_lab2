...
y_zeta <- estimated_dist_zeta(gamma = params_table$gamma_1[params_table$lang == "English"],
                              x = x)
y_real <- real_dist(x)
...
ggplot(data=dist_zeta, aes(x=degree, y=y_pred)) +
...
ggplot(data=dist_zeta, aes(x=degree, y=y_real)) +
  geom_bar(stat="identity", fill = "magenta",) +
  geom_point(colour = "black", fill = "white") +
  theme_minimal() +
  labs(y= "freq",x="k") +
